...
const getTodaysTraffic = async (req, res) => {
    try {
        const result = await adminServices.getTodaysTraffic();
        if (!result) {
            return sendResponse(res, 500, {
                success: false,
                message: "failed to retrieve todays traffic!"
            });
        }
        sendResponse(res, 200, {
            success: true,
...
const getThisMonthsTraffic = async (req, res) => {
    try {
        const result = await adminServices.getThisMonthsTraffic();
        if (!result) {
            return sendResponse(res, 500, {
                success: false,
                message: "failed to retrieve this months traffic!"
            });
        }
        sendResponse(res, 200, {
            success: true,
            message: "this month's traffic retrieved successfully!",
            data: result
        });
    } catch (err) {
        sendResponse(res, 500, {
